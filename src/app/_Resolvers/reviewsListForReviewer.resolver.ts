...
export class reviewsListForReviewerResolver implements Resolve<PaginatedResult<Review[]>> {
    constructor(private reviewService: ReviewService, private router: Router) {}
    resolve(route: ActivatedRouteSnapshot): Observable<PaginatedResult<Review[]>> {
    return this.reviewService.GetReviewerReviews(route.params.reviewerId).pipe(
        catchError(error => {
